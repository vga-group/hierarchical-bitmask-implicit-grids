...
#define REVISION { \
    ++revision; \
    RB_CHECK(static_flag, "Modified static transformable, this is not legal!"); \
}
...
:   orientation(1,0,0,0), position(0), scaling(1), revision(1), static_flag(0)
{
}
...
): transformable()
{
    set_position(pos);
    set_scaling(scale);
    set_orientation(quat_lookat(dir, up));
...
{
    if(static_flag) return cached_revision;
...
void transformable::set_static(bool static_flag)
{
    // We need to update the transform here, because otherwise it won't get
    // updated!
    if(!this->static_flag && static_flag)
        update_cached_transform();
    this->static_flag = static_flag;
}

bool transformable::ancestors_are_static() const
{
    if(!static_flag)
        return false;
    else if(parent)
        return parent->ancestors_are_static();
    return true;
}

transformable_orphan_handler::transformable_orphan_handler(behaviour b)
: b(b) {}

void transformable_orphan_handler::handle(
    scene& ctx, const remove_component<transformable>& e
){
    ...
    ctx([&](entity id, transformable& t){
        if(t.get_parent() == e.data)
        {
            ...
            case REMOVE_ORPHANS:
                ctx.remove(id);
                break;
            case UNPARENT_ORPHANS:
                t.set_parent(nullptr, true);
                break;
            }
        }
    });
